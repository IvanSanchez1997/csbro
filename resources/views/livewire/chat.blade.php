...
<div class="flex h-[550px] text-sm border rounded-xl shadow overflow-hidden bg-white">
    ...
    <div class="w-1/4 border-r bg-gray-50">
        <div class="p-4 font-bold text-gray-700 border-b">Current month's players (All games)</div>
        <div class="divide-y">
            @foreach($users as $user)
                ...
                {{ $selectedUser->id === $user->id ? 'bg-blue-50 font-semibold' : ''}}">
                    ...
                    <div class="text-xs text-gray-500">{{ $user->mail }}</div>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="w-3/4 flex flex-col">
        ...
        <div class="p-4 border-b bg-gray-50">
            ...
            <div class="text-xs text-gray-500">{{$selectedUser->mail}}</div>
        </div>
        ...
        <div class="flex-1 p-4 overflow-y-auto space-y-2 bg-gray-50">
            @foreach ($messages as $message)
                <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                    ...
                    {{ $message->sender_id === auth()->id() ? 'shadow bg-blue-600 text-white' : 'shadow bg-blue-900 text-white' }}">
                        {{ $message->message }}
                    </div>
                </div>
            @endforeach  
